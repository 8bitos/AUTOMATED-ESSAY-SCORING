...
func (h *ClassHandlers) CreateClassHandler(w http.ResponseWriter, r *http.Request) {
	...
	services.PublishNotificationInvalidation("class_created", []string{"teacher"}, nil)
}
...
func (h *ClassHandlers) UpdateClassHandler(w http.ResponseWriter, r *http.Request) {
	...
	services.PublishNotificationInvalidation("class_updated", []string{"teacher", "student"}, nil)
}
...
func (h *ClassHandlers) JoinClassHandler(w http.ResponseWriter, r *http.Request) {
	...
	status, err := h.Service.JoinClass(req.ClassCode, studentID)
	if err != nil {
		...
		if err.Error() == "class is closed for join requests" {
			respondWithError(w, http.StatusForbidden, "Kelas ini tidak menerima join via kode. Minta guru untuk mengundang Anda.")
			return
		}
	...
	message := "Permintaan bergabung berhasil dikirim. Menunggu ACC guru."
	...
	services.PublishNotificationInvalidation(eventType, targetRoles, nil)
}
...
func (h *ClassHandlers) InviteStudentHandler(w http.ResponseWriter, r *http.Request) {
	...
	services.PublishNotificationInvalidation("class_invitation_created", []string{"teacher", "student"}, nil)
}
