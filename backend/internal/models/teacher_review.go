...
type BatchTeacherReviewUpdate struct {
	SubmissionID    string   `json:"submission_id"`
	RevisedScore    *float64 `json:"revised_score,omitempty"`
	TeacherFeedback *string  `json:"teacher_feedback,omitempty"`
}

type BatchTeacherReviewRequest struct {
	Updates []BatchTeacherReviewUpdate `json:"updates"`
}

type BatchTeacherReviewItemError struct {
	SubmissionID string `json:"submission_id"`
	Message      string `json:"message"`
}

type BatchTeacherReviewResponse struct {
	Updated int                           `json:"updated"`
	Failed  []BatchTeacherReviewItemError `json:"failed"`
}
