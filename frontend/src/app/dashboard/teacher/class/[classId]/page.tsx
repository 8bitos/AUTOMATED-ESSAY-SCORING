...
export default function ClassDetailsPage() {
  ...
  const workspaceTabs = [
    ...
    { id: "assessment", label: "Penilaian", badge: String(summary.materialsCount > 0 ? Math.max(0, 100 - summary.reviewProgress) : 0) },
                  ...
                  {collapsedSummaryMode ? (
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      >
                        ...
                        <span className="text-base font-semibold text-slate-900">{material.judul}</span>
                      </button>
                      ...
                      <button
                        ...
                        className="ml-auto sage-button-outline !py-1.5 !px-3 text-xs"
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {!compactDragMode && (
                        <>
                          <div className="flex flex-wrap items-center gap-1.5 rounded-lg border border-slate-200 bg-white p-1">
                            <button
                              type="button"
                            ...
                            <button
                              ...
                              className="sage-button-outline !py-1.5 !px-2.5 text-xs text-red-600 border-red-200 hover:bg-red-50"
                            >
                                  ...
                                  {!isCollapsed && (
                                    <div className="border-t border-slate-200 px-3 py-3">
                                      ...
                                      {card.type === "materi" && card.meta?.materi_mode === "lengkap" ? (
                                        ...
                                        <div className="rounded-md border border-slate-200 bg-slate-50 p-3">
                                          {card.body ? (
                                            <a href={card.body} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm font-medium text-slate-800 underline">
                                              <FiFileText size={14} /> Buka Dokumen
                                            </a>
                                          ) : (
...
function QuickAddSectionContentModal({
  ...
  const [type, setType] = useState<SectionContentType>(initialType || "materi");
  const [title, setTitle] = useState("");
  ...
  const handlePickDocumentFile = (file: File | null) => {
    if (!file) return;
    ...
    if (!hasAllowedExt && !allowedMime.includes(file.type)) {
      setError("Dokumen harus berformat PPT, PDF, atau DOCX.");
      return;
    }
    setError("");
  ...
  const handleUploadDocument = async () => {
    ...
    try {
      ...
      const uploadBody = await uploadRes.json().catch(() => ({}));
      if (!uploadRes.ok) {
        throw new Error(uploadBody?.message || "Gagal upload dokumen.");
      }
      const uploadedPath = typeof uploadBody?.filePath === "string" ? uploadBody.filePath : "";
      if (!uploadedPath) {
        throw new Error("Respons upload dokumen tidak valid.");
      }
      setBody(uploadedPath);
      setUploadedFileName(pendingDocumentFile.name);
              ...
              <div className="mt-1 space-y-3">
                <div className="flex flex-wrap items-center gap-2">
                  <input
                    ...
                    className="hidden"
                    onChange={(e) => handlePickDocumentFile(e.target.files?.[0] || null)}
                  />
                  <button
                    type="button"
                    ...
                    disabled={isUploadingAsset}
                  >
                    Pilih Dokumen (PPT/PDF/DOCX)
                  </button>
                  <button
                    type="button"
                    onClick={() => void handleUploadDocument()}
                    className="sage-button-outline !py-2 !px-3 text-xs"
                    disabled={!pendingDocumentFile || isUploadingAsset}
                  >
                    Upload Sekarang
                  </button>
                </div>
                {pendingDocumentFile && (
                  <div className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs text-slate-600">
                    Preview: {pendingDocumentFile.name} ({Math.ceil(pendingDocumentFile.size / 1024)} KB)
                  </div>
                )}
              ...
              <div className="mt-1 space-y-3">
                ...
                {materiMode === "singkat" ? (
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50 p-3 text-sm text-slate-600">
                      Isi detail materi lengkap dikelola di halaman editor materi.
                    </div>
                  </div>
                )}
              </div>
